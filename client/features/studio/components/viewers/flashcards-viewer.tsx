...
import type { FlashcardsOutputContent } from "@homeworkcopy/contracts";

type Flashcard = FlashcardsOutputContent["cards"][number];

const SWIPE_THRESHOLD = 90;
const SWIPE_VELOCITY = 400;

const staticCardVariants = {
    enter: { x: 0, opacity: 0, scale: 1, rotate: 0 },
    center: { x: 0, opacity: 1, scale: 1, rotate: 0 },
    exit: { x: 0, opacity: 0, scale: 1, rotate: 0 },
};
...
export function FlashcardsViewer({ cards }: { cards: readonly Flashcard[] }) {
            ...
            <div className="relative h-72 perspective-[1600px]">
                ...
                <AnimatePresence initial={false} custom={direction} mode="wait">
                    <motion.div
                        key={position}
                        custom={direction}
                        variants={reduceMotion ? staticCardVariants : cardVariants}
                        initial="enter"
                        animate="center"
                        exit="exit"
                        transition={
                    ...
                    >
                        <motion.div
                            ...
                            aria-label={`Card ${position + 1} of ${cards.length}, showing the ${flipped ? "back" : "front"}. Activate to flip.`}
