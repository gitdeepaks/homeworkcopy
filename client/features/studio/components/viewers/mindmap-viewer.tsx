...
function MindMapCanvas({ nodes, edges, workspaceId }: MindMapCanvasProps) {
    ...
    const tree = useMemo(() => buildMindMapTree(nodes, edges), [nodes, edges]);
...
function MindMapOutline({
    ...
    const rows = useMemo(() => mindMapOutline(nodes, edges), [nodes, edges]);
...
function downloadOutline(nodes: MindMapNode[], edges: MindMapEdge[]) {
    const markdown = mindMapOutline(nodes, edges)
        .map((row) => `${"  ".repeat(row.depth)}- ${row.label}`)
        .join("\n");
    ...
    link.href = url;
    link.download = "mind-map-outline.md";
    ...
    return (
        <div className={containerClass}>
            <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border/60 px-3 py-2">
                ...
                <div className="flex items-center gap-2">
                    <Button
                        size="sm"
                        variant="outline"
                        onClick={() => downloadOutline(nodes, edges)}
                    >
                        <DownloadIcon />
                        Export outline
                    </Button>
            ...
            <div className="min-h-0 flex-1">
                {view === "outline" ? (
                    <MindMapOutline nodes={nodes} edges={edges} />
                ) : (
                    <ReactFlowProvider>
                        <MindMapCanvas
                            nodes={nodes}
